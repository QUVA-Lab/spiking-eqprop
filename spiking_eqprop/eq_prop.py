    ...
    def __call__(self, x_aft=None, x_fore=None, pressure = None, clamp = None):
        ...
        if clamp is not None:
            ...
            if x_fore is not None:
                input_pressure += x_fore @ self.params.w_fore

            de_ds = self.potential - drho(self.potential)*input_pressure
            if pressure is not None:  # Now we apply any external pressures
                de_ds -= pressure
...
def eqprop_update(negative_acts, positive_acts, ws, bs, learning_rate, beta, bidirectional, l2_loss = None):
    ...
    if bidirectional is False:
        w_grads = [-na_pre.t()  @ (pa_post-na_post) / float(n_samples) for na_pre, pa_post, na_post in izip_equal(negative_acts[:-1], positive_acts[1:], negative_acts[1:])]
    elif bidirectional is True:
        w_grads = [-na_pre.t() @ (pa_post-na_post) / float(n_samples) for na_pre, pa_post, na_post in izip_equal(negative_acts[:-1], positive_acts[1:], negative_acts[1:])]
        if bidirectional:
            w_grads = w_grads[:1] + [wg - (pa_pre-na_pre).t() @ na_post / n_samples for wg, pa_pre, na_pre, na_post in izip_equal(w_grads[1:], positive_acts[1:-1], negative_acts[1:-1], negative_acts[2:])]
    else:
        assert bidirectional == 'full'
        w_grads = [-(pa_pre.t() @ pa_post - na_pre.t() @ na_post)/float(n_samples) for na_pre, na_post, pa_pre, pa_post in izip_equal(negative_acts[:-1], negative_acts[1:], positive_acts[:-1], positive_acts[1:])]
